                      ...
                      {grados.map((item, index) => {
                        return (
                          <Option
                            ...
                            active={view === index}
                            color={
                              view === index ? colors.blue.dark : colors.white
                            }
                          >
                            {item.nombre}
                          </Option>
            ...
            {grados.map((grado, index) => {
              ...
              const facultyList = facultades.filter((facultad) => {
                return facultad.carreras.filter((carrera) =>
                  listaDeCarreras.map((item) => item.id).includes(carrera.id),
                ).length
              })

              //Retorna los departamentos que tienen carreras del grado seleccionado.
              const listaDeDepartamentos = departamentos.filter(
                (departamento) => {
              ...
              const esGrado = ['grado'].includes(grado.slug)
                              ...
                              {facultyList.map((facultad) => {
                                return (
                                  <Col key={facultad.id} size={12}>
                                    <Link to={esGrado ? facultad.uri : null}>
                                      <Title
