...
const ProjectLine = ({ data, ...props }) => {
  ...
  const [
    lineaInvestigacion,
  ] = useLineasInvestigacion().filter((lineaInvestigacion) =>
    lines.map((item) => item.id).includes(lineaInvestigacion.id),
  )

  const metaData = {
    title: lineaInvestigacion.nombre,
    description: lineaInvestigacion.copy,
  }

  return (
    <Layout {...props} {...metaData} obj={lineaInvestigacion}>
