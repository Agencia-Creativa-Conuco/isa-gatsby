...
const Investigacion = ({ data, ...props }) => {
  ...
  const [investigacion] = useInvestigaciones().filter((investigacion) =>
    investigaciones.map((item) => item.id).includes(investigacion.id),
  )

  const metaData = {
    title: investigacion.nombre,
    description: investigacion.copy,
  }

  return (
    <Layout {...props} {...metaData} obj={investigacion}>
